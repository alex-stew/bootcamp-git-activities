...
const practiceCoding = new Promise((resolve, reject) => {
    if (studentDistracted) {
        const distract = new Error('Not coding - most likely procrastinating');
        reject(distract);
    } else {
...
practiceCoding.then((resolved) => console.log(resolved));
practiceCoding.catch((distract) => console.error(distract));